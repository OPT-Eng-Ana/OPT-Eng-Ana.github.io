...
- *Fucntion class* $\mathcal{F}$, e.g., convex Lipschitz smooth function.
- *Oracle class* $\mathbb{O}$, e.g., 0-th order oracle (function value), 1-st order oracle (function value and gradient).
- *Algorithm class* $\mathcal{A}$, e.g., a common algorithm class studied in optimization literature is the *linear-span algorithm* interacting with an oracle $\mathbb{O}$, the algorithm is characterized that 
if we let $(x^t)_t$ be the sequence history queries generated by the algorithm, then for the next iterate, we have 
