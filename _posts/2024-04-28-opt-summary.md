...
- *Oracle class* $\mathbb{O}$, for example,
  - Deterministic optimization: first-order oracle (FO, function gradient or subdifferential)
  - Finite-sum optimization: incremental first-order oracle (IFO)<d-cite key="agarwal2015lower"></d-cite><d-cite key="zhou2019lower"></d-cite>
  - Stochastic optimization: stochastic first-order oracle (SFO)<d-cite key="agarwal2009information"></d-cite>

  Here we focus on first-order optimization only, in fact there are also many works on *zeroth-order optimization*, and *higher-order optimization*, and the key difference just lie in the oracle information. For example second-order methods (e.g., Newton's method) generally are able to access the Hessian information, which first-order methods cannot access, so with such finer information, generally second-order methods attain better complexities compared to their first-order counterparts, which is also characterized in theory as mentioned in <d-cite key="carmon2020lower"></d-cite>. 
...
- $M$-Lip Cont.: $M$-Lipschitz continuous.
...
- Here we only consider algorithms equipped with the *generic first-order oracles*, i.e., FO, IFO and SFO mentioned above. So we will not consider algorithms like the proximal point algorthm<d-cite key="rockafellar1976monotone"></d-cite> because their algorithm design reuqires additional operator which is beyond the oracle definition above.
...
1. Here $n$ corresponds to the number of component functions $f_i$, $\sigma^2$ corresponds to the variance of gradient estimator.
2. Shuffling...
...
| SC-C, bilinear, NS    | Duality Gap        | $\Omega(\sqrt{\kappa_x}\epsilon^{-1})$            | $\mathcal{O}(\kappa_x^2\epsilon^{-1/2})$      | [<d-cite key="ouyang2021lower"></d-cite>, Theorem 10], [<d-cite key="chambolle2011first"></d-cite>, Theorem 2]  |? |
...
* Composite Optimization
  ...
  $$ x^{t+1}=\text{prox}_{\lambda r}(x^t-\lambda \nabla f(x^t))  \quad \text{where} \quad \text{prox}_{\lambda r}(z)\triangleq\underset{x}{\arg\min}\left\{r(x)+\frac{1}{2\lambda}\|\|x-z\|\|^2\right\}. $$
